    ...
    data(){
      return {
        ...
        features: [
          ...
          {img: require('../../../assets/img/product/monitor-tubiaozhan.png'), title: '丰富的图标展现形式', desc: '除支持单实例查看多个指标数据；还能单实例多天对比趋势查看，以及选择某个维度聚合查看图表。'},
          {img: require('../../../assets/img/product/monitor-peizhi.png'), title: '自定义配置', desc: '除了新睿云提供的基础告警阈值外，可以根据不同业务需求自定义配置监控告警阈值。新睿云在控制台-用户中心提供了方便的入口，简单的操作，方便用户配置。'},
          {img: require('../../../assets/img/product/monitor-jinggao.png'), title: '异常告警通知', desc: '用户对某个指标特别关注，则可使用提供的告警通知功能。不必每时每刻都关注指标，只用先配置好告警阈值条件，则指标异常后第一时间自动通过邮件、短信方式通知到用户。'}
        ],
        advantages: [
          {
            img: require('../../../assets/img/product/monitor-kaitong.png'),
            title: '自动开通',
            desc: '无需特意购买和开通，申请云服务器，便自动开通了监控告警服务，方便您在购买和使用云服务器后直接查看运行状态。'
          },
          {
            img: require('../../../assets/img/product/monitor-jiangkong.png'),
            title: '多指标监控',
            desc: '为您提供了云服务器 CPU 利用率、内存利用率、磁盘利用率以及弹性IP弹性流量等各项负载和性能指标，通过直观图表展示出来。'
          },
          {
            img: require('../../../assets/img/product/monitor-zidingyijg.png'),
            title: '自定义告警阈值',
            desc: '支持配置多种指标的告警触发阈值。自定义告警接收人和发送渠道。'
          },
          {
            img: require('../../../assets/img/product/montior-feiyong.png'),
            title: '零费用',

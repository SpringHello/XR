    ...
    <modal title="修改伸缩组" v-model="updateTelescopic" width="550" :mask-closable="false">
      ...
      <Form ref="updateTelescopicList" :model="updateTelescopicList" :rules="updateRuleValidate" style="width: 519px"  label-position="top" inline>
        ...
        <FormItem label="防火墙" class="formitem3">
          ...
          <Select v-model="newAddTelescopicList.firewall" style="width:240px" placeholder="选择负载均衡">
            <Option v-for="item in newAddTelescopicList.firewallList" :value="item.value" :key="item.value">{{ item.label }}</Option>
          </Select>
        </FormItem>
      </Form>
      <p style="color: #999999;">提示：伸缩组创建成功之后，请在伸缩组详情页面继续配置告警策略与定时任务，不然伸缩组无法生效</p>
            ...
            render:(h,params)=>{
              ...
              return h('div',[
                ...
                h('span',{
                  ...
                  on:{
                    click:() =>{
                      sessionStorage.setItem('elastic_id',params.row.telescopicgroup);
